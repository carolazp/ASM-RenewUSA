...
#########################################
SECOND POSIBLE MODEL:  regular part: MA(q=2) | seasonal part:  AR(4)_12

```{r}
# ARIMA(0,1,2)(4,1,0)_12
(mod2 <- arima(lnserie, order=c(0,1,2), seasonal=list(order=c(4,1,0),period = 12)))
                  #REGULAR PART:MA(2)     #SEASONAL PART:AR(4)_12     #PERIOD FOR THE SEASONAL PART       
```
all the coefficient are significant.
...
mod2: ARIMA(0,1,2)(4,1,0)_12   ==> -1213.08
mod3: ARIMA(0,1,2)(1,1,1)_12   ==> -1235.35
mod4: ARIMA(1,1,1)(1,1,1)_12   ==> -1231.47
mod5: ARIMA(1,1,1)(0,1,1)_12   ==> -1226.75
mod6: ARIMA(0,1,2)(0,1,2)_12   ==> -1235.66

mod3 and mod6 have the lowest AIC.


EXTRA MODEL:
```{r}
# ARIMA(0,1,2)(1,1,1)_12
(mod3 <- arima(lnserie, order=c(0,1,2), seasonal=list(order=c(1,1,1),period = 12)))
                  #REGULAR PART:MA(2)     #SEASONAL PART:ARMA(1,1)_12     #PERIOD FOR THE SEASONAL PART       
```
all the coeficiant are significant.
This extra model has the lowest AIC.
...
(mod4 <- arima(lnserie, order=c(1,1,1), seasonal=list(order=c(1,1,1),period = 12)))
                  #REGULAR PART:ARMA(1,1)     #SEASONAL PART:ARMA(1,1)_12     #PERIOD FOR THE SEASONAL PART       
```

```{r}
# ARIMA(p,d,q)(P,D,Q)_S
# ARIMA(1,1,1)(0,1,1)_12
(mod5 <- arima(lnserie, order=c(1,1,1), seasonal=list(order=c(0,1,1),period = 12)))
                  #REGULAR PART:ARMA(1,1)     #SEASONAL PART:MA(1)_12     #PERIOD FOR THE SEASONAL PART       
```

```{r}
# ARIMA(0,1,2)(0,1,2)_12
(mod6 <- arima(lnserie, order=c(0,1,2), seasonal=list(order=c(0,1,2),period = 12)))
                  #REGULAR PART:MA(2)     #SEASONAL PART:MA(2)_12     #PERIOD FOR THE SEASONAL PART       
```
```{r}
# checking the residuals for model 6
# we need all the flags lying inside the band: Z_t ~ WN(0, (SIGMA_z)^2 )
par(mfrow=c(1,2))
acf(resid(mod6), ylim=c(-1,1), lag.max = 72,col=c(2,rep(1,11)),lwd=2)
pacf(resid(mod6), ylim=c(-1,1), lag.max = 72,col=c(rep(1,11),2),lwd=2)
par(mfrow=c(1,1))
```
